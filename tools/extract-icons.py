...
sheet showing all icons at 92 px and at the 28 px the graph actually uses.
...
def make_sheet(codes, kinds):
    ...
    apply_sheet_text(
        ...
        f"{len(codes) - vectors} webp) - icon at 92 px and at 28 px",
        13, "#222222",
    )
    ...
    for i, code in enumerate(codes):
        ...
        parts.append(f'<svg x="{x + 108}" y="{y + 4}" width="28" height="28" viewBox="0 0 100 100">{inner}</svg>')
