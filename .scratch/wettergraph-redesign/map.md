...
**Status:** destination reached 2026-09-23 - redesign shipped as 0.5.1 and confirmed on the dashboard. No open tickets; only fog remains.
...
- [Do the webp-backed icons hold at 24 px?](issues/08-icons-at-24px.md)
...
- [Width range for the new layout](issues/04-width-range.md): **`560 .. 1588`, default `794`**, measured with `assets/measure-width-range.py` (output in `assets/width-range.json`). The ticket's premise broke: because `k` scales the font sizes too, the layout is self-similar and **nothing ever collides** - all 30 hour labels survive with clear gaps down to 320 px, and neither axis ever reaches the plot. What fails is **rasterisation**: at 520 the first label's two digits merge, at 480 the text leaves full `#56616c` and the cell grid drops under half contrast; 560 is the last width where every digit splits and the text is full-colour. The ceiling is "nothing gained" rather than a failure - 4000 px still renders in 720 ms - so it is `2 x` the design width, the same rule the old `1564 = 2 x 782` used. Two width-invariant facts fell out and went to [ticket 03](issues/03-rewrite-spec.md): **24 px is the icon ceiling** (the 2 h step clears a 24 px box by 0.47 px, yr's own margin), and a **day label is 69-76 px of ink**, so our current-hour window start makes the first two day labels overlap on any render between roughly 18:00 and 23:00. Note the clamp now only governs the PNG routes and the SVG's intrinsic size - the card scales the SVG itself - but the table reads the same for a browser, so it also answers how small a tile the card can sit in.
...
- [Redraw the renderer to the new spec](issues/05-redraw-renderer.md): `render.py` is redrawn and `tools/render-check.py` proves it with **91 checks, all passing**. The check does not re-derive the layout, it **diffs the renderer's own SVG against [the reference](issues/01-layout-reference.md)**, and on yr's own forecast the two agree element for element - 72 grid lines, both separators, 30 hour labels, 5 + 5 axis labels, both glyphs, the legend, a 60-segment spline, 30 icon boxes, and (against the `-rain` file) all 14 bars including the clipped 13.4 mm/h one. **One constant separates them and it is §5.1 working**: the reference hard-codes yr's band `3..33`, the ladder fits ours to `6..36` on the same data, so every `y` differs by exactly 12 px; the check reads both bands off the files' own labels and carries it rather than hiding it. The ladder itself is proved on **yr's own dark meteogram** - fed that file's curve it returns `(4, 2)`, the band yr's own dark labels state. Four things the spec left to the implementation are recorded there: `k` is the **`viewBox`**, not a per-number multiplication (same picture, no rounding noise, and how ticket 04 measured); the §7.5 chip runs `y = 0..12`; a zero bar is not drawn at all; gradient offsets are written raw and left to SVG's own clamp. Samples for all five cases are committed (`assets/render-sample-*.{svg,png}`, rebuilt by `make-samples.py`), the first four on the real met.no payload at a render hour 13 h past its fetch, so §4.8's interpolated tail is exercised everywhere. The shape change also forced `server.py` (height, the 30-icon self-check, the width probe), `config.yaml` (`794`, `560..1588`) and the README; one render now costs **~140 ms at 794 px**, twice the retired layout.
- [Ship the new look and confirm it on the dashboard](issues/06-ship-and-confirm.md): **shipped as 0.5.1, operator confirmed** - both themes match the reference, legible on the phone, and ticket 07's card YAML is final unchanged (`columns: 18`, `rows: auto` fits the ~3.8:1 shape; no option default moves). 0.5.0 drew its hour labels in **UTC** (a 12:33 render labelled `10`): Supervisor's `TZ` never reaches the process, so `localzone.py` now takes HA's own `time_zone` from `.storage/core.config` over the existing mapping - the box's log confirms that fallback is what carries it - and a startup check fails on UTC. Refresh accepted from the log's rewrites rather than watched.
- [Do the webp-backed icons hold at 24 px?](issues/08-icons-at-24px.md): **all 83 hold, no fix.** The contact sheet (`.scratch/wettergraph-ha-widget/assets/icon-contact-sheet.png`, from `tools/extract-icons.py`) now shows each code at 24 px on a light and a dark cut of the real plot, grid included, placed off the pixel grid as a render places it. The operator found the grey night moon "a bit dark on dark" and accepted it; a dark-only halo filter (cheap) and yr's dark moon tint (a second raster set for ~26 codes) were weighed and declined, so §6.5 and §9.3 stand.

## Not yet specified

<!-- in-scope fog: suspected questions not yet sharp enough to ticket -->

- **What the `/local/` SVG leaves behind.** The mapping, the writer and the caching are settled ([ticket 07](issues/07-local-svg-publish.md)). What stays fog: whether the Generic Camera and the `/share` PNG are worth keeping once the dashboard reads the SVG, or whether they become dead weight nobody looks at. Not answerable until the operator has lived with the SVG card. Also here: [ticket 06](issues/06-ship-and-confirm.md) found something on the LAN (`10.0.1.145`) speaking TLS to plain-HTTP port 8099, 177 `400` lines per log window - probably a leftover `https://...:8099` camera or card, which goes away with whatever answers this. The operator's working card, against yr's own URL, was the pattern ticket 07 copied:
...
- **How the fitted °C band reads.** The ladder of §5.1 puts the band bottom at `floor(t_min/r)*r`, so a flat window can leave the curve hugging the plot's bottom line, and the band's step changes between renders as the forecast moves. Whether that looks right - or wants a minimum bottom margin, or a stickier step - is not answerable until the operator has seen a few days of real renders (first seen on 0.5.1, [ticket 06](issues/06-ship-and-confirm.md)).
